...
def firstImplict(v0,initialData,thish,thisk):
	currentSol=[]
	lastSol=[]
	nextSol=[]
	tmax=300
	...
	while(currentStep<=tmax):
		...
		if(currentStep==0):
			lastSol=initialData
			currentSol=np.linalg.inv(currentMatrix)*(C*initialData)
			createNewLineSolution(wholeSolution,currentStep,y,currentSol)
		#general case of updating after first step
		else:
			newSol=np.linalg.inv(currentMatrix)*(C*currentSol+B*lastSol)
			lastSol=currentSol
			currentSol=newSol
...
firstImplict(8.7,IC1,h,k)
firstImplict(8.7,IC2,h,k)
firstImplict(8.7,IC3,h,k)
firstImplict(20,IC1,h,k)
firstImplict(20,IC2,h,k)
firstImplict(20,IC3,h,k)
firstImplict(200,IC1,h,k)
firstImplict(200,IC2,h,k)
firstImplict(200,IC3,h,k)
